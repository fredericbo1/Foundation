...
%sql% -d %cms_db% -b -i "packages\%cms_core%\tools\EPiServer.Cms.Core.sql" 

echo ## Installing Commerce database ##
::echo ## Installing Commerce database ## >> Build\Logs\Database.log
%sql% -d %commerce_db% -b -i "packages\%commerce_core%\tools\EPiServer.Commerce.Core.sql" 

echo ## Installing EPiServer.Find.Cms schema ##
::echo ## Installing EPiServer.Find.Cms schema ## >> Build\Logs\Database.log
%sql% -d %cms_db% -b -i "packages\%find_cms%\tools\epiupdates\sql\1.0.1.sql" 
%sql% -d %cms_db% -b -i "packages\%find_cms%\tools\epiupdates\sql\12.2.8.sql"
%sql% -d %cms_db% -b -i "packages\%find_cms%\tools\epiupdates\sql\12.4.2.sql"
%sql% -d %cms_db% -b -i "packages\%find_cms%\tools\epiupdates\sql\13.0.4.sql"

echo ## Installing EPiServer.Marketing schema ##
::echo ## Installing EPiServer.Marketing schema ## >> Build\Logs\Database.log
%sql% -d %cms_db% -b -i "packages\%testing_core%\tools\epiupdates\sql\1.0.0.1.sql" 
%sql% -d %cms_db% -b -i "packages\%testing_core%\tools\epiupdates\sql\1.0.0.2.sql" 
%sql% -d %cms_db% -b -i "packages\%testing_core%\tools\epiupdates\sql\1.0.0.3.sql" 
%sql% -d %cms_db% -b -i "packages\%testing_core%\tools\epiupdates\sql\1.0.0.4.sql" 
%sql% -d %cms_db% -b -i "packages\%testing_core%\tools\epiupdates\sql\1.0.0.5.sql" 
%sql% -d %cms_db% -b -i "packages\%testing_core%\tools\epiupdates\sql\1.0.0.6.sql" 
%sql% -d %cms_db% -b -i "packages\%testing_core%\tools\epiupdates\sql\1.0.0.7.sql" 

echo ## Installing ASP.NET Identity ##
::echo ## Installing ASP.NET Identity ## >> Build\Logs\Database.log
%sql% -d %commerce_db% -b -i "Build\SqlScripts\aspnet_identity.sql" 

echo ## Installing foundation configuration ##
::echo ## Installing foundation configuration ## >> Build\Logs\Database.log
%sql% -d %commerce_db% -b -i "Build\SqlScripts\FoundationConfigurationSchema.sql" -v appname=%APPNAME% foundationhostname=%FOUNDATIONDOMAIN% cmhostname=%CMDOMAIN% 

echo ## Installing unique coupon schema ##
::echo ## Installing unique coupon schema ## >> Build\Logs\Database.log
%sql% -d %commerce_db% -b -i "Build\SqlScripts\UniqueCouponSchema.sql" 

echo ## Installing Service API CMS ##
::echo ## Installing Service API CMS ## >> Build\Logs\Database.log
%sql% -d %cms_db% -b -i "Build\SqlScripts\ServiceApiCms.sql" 

echo ## Installing Service API Commerce ##
::echo ## Installing Service API Commerce ## >> Build\Logs\Database.log
%sql% -d %commerce_db% -b -i "Build\SqlScripts\ServiceApiCommerce.sql"
